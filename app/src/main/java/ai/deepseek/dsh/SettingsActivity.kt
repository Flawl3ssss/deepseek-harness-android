...
/**
 * Настройки (ARCH-MOD-09, TASK-010): zen-ключ (ESP), модель, локаль, AMOLED, шрифт,
 * RAM Node, час бэкапа + Backup now, пин-версия (R-01), обновления по кнопке (R-02),
...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // apply/rollback — TASK-014: кнопки появятся здесь же.
        col.addView(btn(R.string.set_updates) {
            startActivity(
                Intent(
                    Intent.ACTION_VIEW,
                    Uri.parse("https://github.com/Flawl3ssss/deepseek-harness-android/releases")
                )
            )
        })
        col.addView(btn(R.string.action_send_log) { InstallLog.share(this) })
        col.addView(Button(this).apply {
            text = "Wipe & reinstall"
            setOnClickListener {
                AlertDialog.Builder(this@SettingsActivity)
                    .setMessage("Удалить rootfs+payload и скачать заново? Бэкапы сохранятся.")
                    .setPositiveButton("Да") { _, _ ->
                        Thread {
                            try {
                                Paths.rootfsDir(this@SettingsActivity).deleteRecursively()
                                Paths.payloadDir(this@SettingsActivity).deleteRecursively()
                                Paths.nodeDir(this@SettingsActivity).deleteRecursively()
                            } catch (_: Exception) {
                            }
                            runOnUiThread {
                                startActivity(Intent(this@SettingsActivity, BootActivity::class.java))
                                finish()
                            }
                        }.start()
                    }
