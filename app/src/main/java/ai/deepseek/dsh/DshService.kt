...
/**
 * ForegroundService (ARCH-MOD-01, DEC-004, R-10): specialUse, 2 канала, START_STICKY,
 * запуск zen→DSH с health-gate, WakeLock только на задачу.
 * v1.1: watchdog-poll 15с (TASK-101) — отдельно, не в этом файле.
 */
class DshService : Service() {
    ...
    private var dshUrl: String = "http://127.0.0.1:${Proot.DSH_PORT}/"
    ...
    private fun boot() {
        ...
        InstallLog.w(this, "service", "service boot (dsh_pin=0.1.7-alpha.1)")
        ...
        try {
            ...
            if (!Proot.waitPort(Proot.ZEN_PORT)) {
                ...
                InstallLog.w(this, "service", "adapter health check FAILED")
            }
            // 2. dsh web. Токен из `dsh web:` строки НЕ логируем (RISK-005) —
            // перехватываем URL отдельно через parseWebUrl (см. MainActivity).
            dsh = Proot.startDaemon(
                this, "dsh-web",
                Proot.full(this, prefs, Proot.dshArgv(), "/root/workspace"),
                File(logs, "dsh.log")
            )
            ...
            InstallLog.w(this, "service", "service boot FAILED: ${e.message}")
            notifyFail("Start error: ${e.message}")
        }
    }

    fun webUrl(): String = dshUrl
    ...
    override fun onDestroy() {
        ...
        InstallLog.w(this, "service", "service destroyed")
        super.onDestroy()
    }
}
