...
/**
 * Главный экран (ARCH-MOD-01/04, DEC-001): WebView на локальный DSH web (127.0.0.1:3080),
 * JS-мост inbox (DSHInbox.list/path), file-chooser с копией в inbox,
 * меню: Terminal/Editor/Files/Logs/Settings.
 *
 * Process token из `dsh web:` URL живёт только в памяти WebView, в логи не пишется (RISK-005).
 */
class MainActivity : AppCompatActivity() {
    ...
    private fun applyTheme() {
        androidx.appcompat.app.AppCompatDelegate.setDefaultNightMode(
            if (Prefs(this).amoled()) androidx.appcompat.app.AppCompatDelegate.MODE_NIGHT_YES
    ...
    private fun setupWeb() {
        ...
        web.settings.apply {
            ...
            userAgentString = "$userAgentString DSH-Android/${ai.deepseek.dsh.BuildConfig.VERSION_NAME}"
        }
        web.addJavascriptInterface(InboxBridge(), "DSHInbox")
        web.webViewClient = object : WebViewClient() {
            override fun shouldOverrideUrlLoading(view: WebView, req: WebResourceRequest): Boolean {
                val url = req.url.toString()
                // Всё за пределами loopback-DSH — наружу в системный браузер.
    ...
    @Deprecated("file chooser result")
    ...
    private fun copyToInbox(uri: Uri) {
        try {
            ...
            InstallLog.w(this, "web", "inbox copy failed: ${e.message}")
        }
    }
